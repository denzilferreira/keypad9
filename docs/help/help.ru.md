...
## Начальная настройка
После установки вам нужно сначала включить Традиционный T9 как клавиатуру Android. Для этого нажмите на значок запуска. Если необходимо предпринять какие-либо действия, все опции, кроме Начальной Настройки, будут отключены, и появится надпись: "Keypad9 отключен". Перейдите в Начальную Настройку и включите его.
...
Другая проблема заключается в том, что проверка орфографии может отображать сбивающее с толку всплывающее окно «Добавить слово». Это окно добавляет новые слова в стандартную системную клавиатуру (обычно Gboard), а не в словарь Keypad9.
...
Загрузка словарей может перегружать слабые телефоны. При использовании версии Keypad9 "lite" это может привести к тому, что Android прервёт операцию. Если загрузка занимает больше 30 секунд, подключите зарядное устройство или убедитесь, что экран остаётся включённым во время загрузки.

Вы можете избежать этого, используя вместо этого версию "full".

#### Примечания для Android 13 или выше
По умолчанию уведомления для недавно установленных приложений отключены. Рекомендуется их включить. Таким образом, вы получите уведомление, когда будут обновления словарей, и как только вы решите их установить, Keypad9 покажет прогресс загрузки. Новые обновления выпускаются не чаще одного раза в месяц, поэтому не беспокойтесь о чрезмерном количестве уведомлений.

Вы можете включить уведомления, перейдя в Настройки → Языки и включив Уведомления о словарях.

_Если вы решите оставить уведомления отключёнными, Keypad9 продолжит работать без проблем, но вам придётся управлять словарями вручную._
...
#### Способ 1
Нажмите на значок приложения Keypad9.

#### Способ 2 (с использованием сенсорного экрана)
- Нажмите на текстовое или числовое поле, чтобы активировать Keypad9.
- Используйте экранную кнопку шестерёнки.

#### Способ 3 (с использованием физической клавиатуры)
- Начните ввод текста или цифр в поле, чтобы активировать Keypad9.
...
#### Добавленные слова
Опция «Экспорт» позволяет создать файл CSV со всеми добавленными словами для всех языков. Затем вы можете использовать этот файл CSV, чтобы улучшить Keypad9! Перейдите на GitHub и поделитесь словами в [новом issue](https://github.com/sspanak/tt9/issues) или [pull request](https://github.com/sspanak/tt9/pulls). После проверки и одобрения они будут включены в следующую версию.
...
_**Примечание:** Каждое приложение само решает, что делать при нажатии OK, и Keypad9 не контролирует это._
...
_История отмен управляется самими приложениями, а не Keypad9. Это означает, что функция отмены может быть недоступна в некоторых приложениях._
...
У многих телефонов есть только две или три «свободные» клавиши, которые можно использовать как горячие клавиши. Однако в Keypad9 доступно намного больше функций, что означает, что просто нет места для всех этих функций на клавиатуре. Пульт команд решает эту проблему, позволяя вызывать дополнительные функции (или команды) с помощью комбинаций клавиш.
...
- **Горизонтальное смахивание:** Переключение на последнюю использованную клавиатуру, кроме Keypad9.
...
_Изменение размера Keypad9 также изменяет размер текущего приложения. Оба процесса требуют значительных вычислительных ресурсов. Это может вызвать мерцание или задержки на многих телефонах, даже на более мощных._
...
Значение по умолчанию 100% является хорошим балансом между размером кнопок и занимаемым экранным пространством. Однако, если у вас большие пальцы, вы можете немного увеличить этот параметр, а если вы используете Keypad9 на большом экране, например, на планшете, вы можете уменьшить его.

_Если доступное экранное пространство ограничено, Keypad9 проигнорирует этот параметр и автоматически уменьшит высоту, чтобы оставить достаточно места для текущего приложения._
...
## Голосовой ввод
Голосовой ввод позволяет преобразовывать речь в текст, аналогично Gboard. Как и другие клавиатуры, Keypad9 не выполняет распознавание речи самостоятельно, а использует голосовые службы Android.
...
На устройствах с сервисами Google Keypad9 использует инфраструктуру Google для преобразования речи в текст. В Android 12 и ниже необходимо подключение к Wi-Fi или включенные мобильные данные. В Android 13 и выше Keypad9 может выполнять распознавание речи как онлайн, так и офлайн, используя языковые пакеты устройства. Для работы в офлайн-режиме убедитесь, что вы загрузили нужные языки: Настройки Android → Система → Распознавание на устройстве → Добавить язык.

_Языковые пакеты, установленные для Google Voice, других голосовых помощников или клавиатур, не гарантированно работают с Keypad9. Рекомендуется устанавливать глобальные пакеты через экран "Распознавание на устройстве"._
...
3. Предоставьте Keypad9 разрешение на использование микрофона.
...
Keypad9 также поддерживает негoogle-овские методы голосового ввода, такие как Futo Voice или Sayboard. Если доступно несколько методов, Keypad9 предложит выбрать один из них. Если установлен только один негoogle-овский метод, он будет использоваться автоматически.

## Устранение неполадок
Для нескольких приложений или устройств возможно включить специальные опции, которые сделают Keypad9 более совместимым с ними. Эти параметры можно найти в конце каждого экрана настроек в разделе Совместимость.
...
_**Примечание:** Чем выше значение, тем медленнее вам придётся набирать текст. Keypad9 будет игнорировать очень быстрые нажатия клавиш._
...
Проблема возникает из-за того, что Deezer и Smouldering Durtles иногда изменяют введённый текст, что приводит к сбоям в работе Keypad9.
...
### Keypad9 не появляется сразу в некоторых приложениях (только для телефонов без сенсорного экрана)
Если вы открыли приложение, где можно вводить текст, но Keypad9 не появляется автоматически, просто начните вводить, и он появится. Альтернативно, нажатие горячих клавиш для смены [режима ввода](#клавиша-смены-режима-ввода-по-умолчанию-нажмите) или [языка](#клавиша-смены-языка-по-умолчанию-удерживайте) также может вызвать Keypad9, если он скрыт.

На некоторых устройствах Keypad9 может оставаться невидимым, что бы вы ни делали. В таких случаях вам придётся включить [Всегда сверху](#всегда-сверху).

**Длинное объяснение.** Проблема заключается в том, что Android в основном предназначен для устройств с сенсорным экраном. Поэтому он ожидает, что вы коснётесь текстового/числового поля, чтобы отобразить клавиатуру. Можно настроить Keypad9 так, чтобы он появлялся без этого подтверждения, но в некоторых случаях Android забудет скрыть его, когда это необходимо. Например, он может остаться видимым после того, как вы набрали номер телефона или отправили текст в поисковом поле.
...
Чтобы решить эту проблему, перейдите в Настройки → Внешний вид и включите «Значок состояния». Keypad9 должен автоматически обнаружить Qin F21 и включить настройки, но если автоопределение не сработало или вы отключили значок по какой-либо причине, вам необходимо включить его, чтобы все клавиши работали корректно.

**Длинное объяснение.** Qin F21 Pro (и, возможно, F22) имеет приложение горячих клавиш, которое позволяет назначать функции увеличения и уменьшения громкости на цифровые клавиши. По умолчанию менеджер горячих клавиш включён, и удерживание клавиши 2 увеличивает громкость, удерживание клавиши 8 — уменьшает. Однако, когда нет значка состояния, менеджер предполагает, что клавиатура не активна, и регулирует громкость, вместо того чтобы позволить Keypad9 обрабатывать клавиши для набора чисел. Поэтому включение значка обходит менеджер горячих клавиш, и всё работает нормально.

### Общие проблемы на телефонах Xiaomi
Xiaomi ввела несколько нестандартных разрешений на своих телефонах, которые мешают корректной работе виртуальной экранной клавиатуры Keypad9. Точнее, клавиши «Показать настройки» и «Добавить слово» могут не выполнять свои функции. Чтобы исправить это, необходимо предоставить Keypad9 разрешения «Отображать всплывающее окно» и «Отображать всплывающее окно в фоновом режиме» в настройках вашего телефона. [Этот гид](https://parental-control.flashget.com/how-to-enable-display-pop-up-windows-while-running-in-the-background-on-flashget-kids-on-xiaomi) для другого приложения объясняет, как это сделать.
...
Это [известная проблема](https://issuetracker.google.com/issues/158198432) на Android 10, которую Google так и не исправил. Устранить её на стороне Keypad9 невозможно. Чтобы завершить голосовой ввод, оставайтесь молчаливыми несколько секунд. Android автоматически отключает микрофон, когда не может обнаружить речь.

### Моё банковское приложение не принимает Keypad9
Это не является проблемой Keypad9. Банки часто ограничивают использование нестандартных или открытых клавиатур, поскольку не хотят рисковать и предполагают, что они могут быть небезопасными. Некоторые идут ещё дальше и предоставляют собственные клавиатуры, блокируя даже стандартную клавиатуру Google — Gboard. К сожалению, в данном случае единственным вариантом является переключение на стандартную клавиатуру устройства.

### Вибрация не работает (только сенсорные экраном)
Функции энергосбережения, оптимизации и режим "Не беспокоить" могут блокировать вибрацию. Проверьте в системных настройках вашего устройства, активированы ли какие-либо из этих параметров. На некоторых устройствах можно настроить параметры оптимизации батареи отдельно для каждого приложения через Системные настройки → Приложения. Если ваше устройство поддерживает это, отключите оптимизацию для Keypad9.
...
В конечном итоге вибрация может не работать на некоторых устройствах. Для исправления этой проблемы потребуется больше разрешений и доступ к дополнительным функциям устройства. Однако, поскольку Keypad9 является клавиатурой, ориентированной на конфиденциальность, он не будет запрашивать такой доступ.
...
Генри Форд однажды сказал: «Покупатель может выбрать любой цвет, при условии, что он будет черным». Так же и Keypad9 – это простой, эффективный и бесплатный продукт, и он остается таким, как есть.
...
Keypad9 не предназначен для того, чтобы быть заменой или клоном какого-либо приложения. Он имеет уникальный дизайн, вдохновленный в первую очередь Nokia 3310 и 6303i. И хотя он передает ощущение классических телефонов, он предлагает свой собственный опыт, который не копирует ни одно устройство полностью.
...
Это стандартное предупреждение Android, которое отображается при установке и активации любой клавиатуры, а не только Keypad9. Можете быть уверены, что всё, что вы вводите, остаётся на вашем устройстве. Механизм ввода полностью с открытым исходным кодом; вы можете ознакомиться с его кодом на GitHub и убедиться, что ваша конфиденциальность надёжно защищена.
...
Keypad9 — это клавиатура типа T9, и, соответственно, она не предоставляет раскладку, похожую на QWERTY.
...
Большинство других клавиатур позволяет переключиться обратно на Keypad9, удерживая пробел или клавишу «смены языка». Подробнее см. в соответствующем руководстве или инструкции.
